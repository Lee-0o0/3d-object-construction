...
public class Smoothing {
    public static List<LineSegment> smoothing(List<LineSegment> lineSegments) {
        ...
        double threshold = width*lineSegments.size()/10;
        double movingDistance = 10000000.0;
        double ratio = 0.4;
        boolean[] isVisited = new boolean[lineSegments.size()];

        List<LineSegment> computedLineSegment = new ArrayList<>();
        for (LineSegment lineSegment:lineSegments){
            LineSegment lineSegment1 = new LineSegment(lineSegment.getFirst(),lineSegment.getSecond());
...
//                        System.out.println("next："+next.getFirst()+"--"+next.getSecond());
                        ...
                        if (last.getFirst().equals(next.getFirst())) {
                            points[0] = last.getSecond();
                            points[1] = last.getFirst();
                            points[2] = next.getSecond();
                        } else if (last.getFirst().equals(next.getSecond())) {
                            points[0] = last.getSecond();
                            points[1] = last.getFirst();
                            points[2] = next.getFirst();
                        } else if (last.getSecond().equals(next.getFirst())) {
                            points[0] = last.getFirst();
                            points[1] = last.getSecond();
                            points[2] = next.getSecond();
                        } else if (last.getSecond().equals(next.getSecond())) {
                            points[0] = last.getFirst();
                            points[1] = last.getSecond();
                            points[2] = next.getFirst();
                        }
...
//                        System.out.println(newPoint);
                        // 更新移动后的点，注意更新两条线上的点
                        if (points[1].equals(last.getFirst())){
                            computedLineSegment.get(indexOfLast).setFirst(newPoint);
                        }else {
                            computedLineSegment.get(indexOfLast).setSecond(newPoint);
                        }
                        if (points[1].equals(next.getFirst())) {
                            computedLineSegment.get(indexOfNext).setFirst(newPoint);
                        }else {
                            computedLineSegment.get(indexOfNext).setSecond(newPoint);
                        }
            ...
            for (int i = 0; i <lineSegments.size(); i++) {
                ...
                lineSegment.setFirst(computedLineSegment.get(i).getFirst());
                lineSegment.setSecond(computedLineSegment.get(i).getSecond());
            }
        }
