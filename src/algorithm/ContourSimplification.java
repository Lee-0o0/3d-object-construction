...
import entity.LineDistortionError;
            ...
            for (int i = 0; i < regions.size(); i++) {
                double[] proxy =
                        MathUtil.getLineFromTwoPoints(
                                regions.get(i).get(0).getFirst(),
                                regions.get(i).get(0).getSecond());
                proxies.add(proxy);
            }
...
//            }
            ...
            List<LineSegment> verification = verification(points,startEnd, lineSegments, 0, lineSegments.length - 1);
        ...
        for (LineSegment lineSegment : set) {
            if (!lineSegment.getFirst().equals(lineSegment.getSecond())) {
    ...
    private static List<LineSegment> verification(List<Point> points,Point[] startEnd,LineSegment[] lineSegments,int begin,int end){
...
//                System.out.println("第"+i+"条边开始检查。");
                Point first = lineSegments[i].getFirst();
...
//                System.out.println("first点所在的边="+edge1);
                ...
                if (!MathUtil.isCrossALine(line,edge1)){
                    ...
                    newPoints1[1] = first;
                    List<LineSegment> one = verification(points,newPoints1, lineSegments, begin1, end1);
                    Point[] newPoints2 = new Point[2];
                    newPoints2[0] = first;
                    newPoints2[1]=startEnd[1];
                    List<LineSegment> two = verification(points,newPoints2, lineSegments, begin2, end2);
                    res.addAll(one);
                    res.addAll(two);
                    return res;
                }
                Point second = lineSegments[i].getSecond();
...
//                System.out.println("误差"+totalDistortion);
     ...
     * @return
     */
    private static List<LineSegment> extractOrderedLinesFromARegion(List<LineSegment> region,List<Point> points){
    ...
    public static List<Point> extractPointsFromARegion(List<LineSegment> region){
        ...
        for (LineSegment lineSegment : region ) {
            points.add(lineSegment.getFirst());
            points.add(lineSegment.getSecond());
        }
        // 查找源头点
//        System.out.println("查找起始点");
        ...
        for (Point point:points){
            ...
            for (LineSegment lineSegment:region){
                if (lineSegment.getFirst().equals(point) || lineSegment.getSecond().equals(point)){
...
//        System.out.println("点有序");
        ...
        Point next = startLine.getFirst().equals(start)?startLine.getSecond():startLine.getFirst();

        while (orderedPoints.size()!=region.size()){
            for (LineSegment lineSegment:region){
                if (orderedPoints.contains(lineSegment.getFirst())
                        || orderedPoints.contains(lineSegment.getSecond())) {
                    continue;
                }else {
                    if (lineSegment.getFirst().equals(next)){
                        orderedPoints.add(next);
                        next = lineSegment.getSecond();
                        break;
                    }
                    if (lineSegment.getSecond().equals(next)){
                        orderedPoints.add(next);
                        next = lineSegment.getFirst();
                        break;
                    }
                }
            }
    ...
    private static double distortionErrorBetweenTwoLines(LineSegment line, double[] proxy) {
        double d0 = MathUtil.distanceBetweenPointAndLine(proxy, line.getFirst());
        double d1 = MathUtil.distanceBetweenPointAndLine(proxy, line.getSecond());
