...
public class StepUtil {
    ...
    public static Point computeBarycenter(List<LineSegment> region){
        ...
        for (LineSegment lineSegment:region){
            ...
            memberX += length*(lineSegment.getStartPoint().getX()+lineSegment.getEndPoint().getX());
            memberY += length*(lineSegment.getStartPoint().getY()+lineSegment.getEndPoint().getY());
    ...
    public static Matrix computeCovarianceMatrix(List<LineSegment> region, Point xi){
        ...
        for (LineSegment lineSegment:region){
            ...
            ve.setAsDouble(lineSegment.getStartPoint().getX(),0,0);
            ve.setAsDouble(lineSegment.getStartPoint().getY(),1,0);
            // 矩阵vs
            DenseMatrix vs = Matrix.Factory.zeros(2, 1);
            vs.setAsDouble(lineSegment.getEndPoint().getX(),0,0);
            vs.setAsDouble(lineSegment.getEndPoint().getY(),1,0);
            // 矩阵vs的转置
