...
public class MathUtil {
    ...
    public static Point intersectionOfTwoLineSegment(LineSegment a,LineSegment b){
        ...
        double x1 = a.getStartPoint().getX();
        double x2 = a.getEndPoint().getX();
        double x = intersection.getX();
        double res = ArithUtil.mul(ArithUtil.sub(x, x1), ArithUtil.sub(x, x2));
    ...
    public static List<LineSegment> createContour(List<Point> points){
        List<LineSegment> contour = new ArrayList<>();
        for (int i = 0; i < points.size(); i++){
            if (i < points.size() - 1) {
                contour.add(new LineSegment(points.get(i), points.get(i + 1)));
            }else if(i == points.size()-1){
                contour.add(new LineSegment(points.get(i), points.get(0)));
            }
        }
        return contour;
    }
