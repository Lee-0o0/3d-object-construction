...
import com.lee.paint.DrawingPaint;
...
public class Main {
    ...
    public static void mainFunction(
        ...
        // 插值，增加层数
        data = Interpolation.linearInterpolate(data, layerHeight);

        int layers = data[0][0].length;
        ...
        if (data.length * data[0].length * data[0][0].length * 8 * 16L > totalMemory*3.0 / 4.0) {
            ...
            for (int i = 0; i < data[0][0].length; i++) {
                ...
                // 单层四叉树插值
                double[][] layerData = QuadtreeRecursion.layerQuadtree(data, i, accuracy);
                // 转置
